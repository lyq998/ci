function [r,s] = algorithm2()
    ...
    the_best_group_in_each_gen = zeros(max_gen, max_gen);

    %% initialize the population
    pops = rand(pop_size, 2)*(x_max-x_min) - x_max;

    for curr_gen = 1:max_gen
        %% fitness evaluation
        fitness = dejong2(pops(:,1), pops(:,2));
        ...
        for i = 1:2:pop_size
            ...
            for j = 1:size(pops, 2)
                if rand() <= prob_crossover
                    [z1, z2] = sbxnew(pops(p1, j), pops(p2, j), x_min, x_max);
                    offspring1(j) = z1;
                    offspring2(j) = z2;
                else
            ...
            for j = 1:size(pops, 2)
                if rand() <= prob_mutation
                    z = polynomial_mutation_new(offspring1(j), x_min, x_max);
                else
                    z = offspring1(j);
                end
                offspring1(j) = z;
            ...
            for j = 1:size(pops, 2)
                if rand() <= prob_mutation
                    z = polynomial_mutation_new(offspring2(j), x_min, x_max);
                else
                    z = offspring2(j);
                end
                offspring2(j) = z;
        ...
        fitness_offspring = dejong2(offspring(:,1), offspring(:,2));
        ...
        the_best_group_in_each_gen(:, curr_gen) = total_fitness(I(1:max_gen));
